<img src="../img/asyncpy图标.png"></img>

## 简介

```stylus
Asyncpy是基于asyncio和aiohttp开发的一个轻便高效的爬虫框架，采用了scrapy的设计模式，开发者参考了github上一些开源框架的处理逻辑
github: https://github.com/lixi5338619/asyncpy
pypi: https://pypi.org/project/asyncpy/
```

<img src="../img/asyncpy官方.png"></img>

## 架构原理流程

```stylus
在scrapy的基础上，省略掉了爬虫中间件，其他原理则是相同的<在这里原理指的是一个url地址请求数据传递和信号传递原理>
```

<img src="../img/架构原理流程.png"></img>
...
注意：此命令执行后，会创建一个名为demo的项目文件夹，项目结构与scrapy相似，爬虫程序文件为demo.py
```
...
```stylus
# 如果你定义了item(目前只支持dict字典格式的item)，并且settings 里面 启用了pipelines 那么你就可以在pipelines 里面 编写 连接# 数据库，插入数据的代码。
```
